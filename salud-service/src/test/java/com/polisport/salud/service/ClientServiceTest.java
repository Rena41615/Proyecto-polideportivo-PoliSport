...
/**
 * Pruebas unitarias de {@link ClientService}, simulando la cadena fluida del
 * {@link WebClient} para verificar las operaciones GET y POST sin realizar
...
class ClientServiceTest {
    ...
    void obtenerDatosDebeRetornarLaRespuestaCuandoLaPeticionEsExitosa() {
    ...
    @Test
    @SuppressWarnings("unchecked")
    void obtenerDatosDebePropagarElErrorCuandoLaPeticionFalla() {
        when(webClient.get()).thenReturn(requestHeadersUriSpec);
        when(requestHeadersUriSpec.uri(anyString())).thenReturn(requestHeadersSpec);
        when(requestHeadersSpec.retrieve()).thenReturn(responseSpec);
        ...
        RuntimeException excepcion = org.junit.jupiter.api.Assertions.assertThrows(
                RuntimeException.class, resultado::block);
        assertEquals("error de conexion", excepcion.getMessage());
    }

    @Test
    @SuppressWarnings("unchecked")
    void enviarDatosDebeRetornarLaRespuestaCuandoLaPeticionEsExitosa() {
    ...
    @Test
    @SuppressWarnings("unchecked")
    void enviarDatosDebePropagarElErrorCuandoLaPeticionFalla() {
        when(webClient.post()).thenReturn(requestBodyUriSpec);
        when(requestBodyUriSpec.uri(anyString())).thenReturn(requestBodySpec);
        when(requestBodySpec.bodyValue(any())).thenReturn(requestHeadersSpec);
        when(requestHeadersSpec.retrieve()).thenReturn(responseSpec);
        ...
        RuntimeException excepcion = org.junit.jupiter.api.Assertions.assertThrows(
                RuntimeException.class, resultado::block);
        assertEquals("error al enviar", excepcion.getMessage());
    }
}
